<?php
    ...
    if(isset($_GET['id']) && $_GET['id'] != '')
    {
        $id = $_GET['id'];
        $paramList = [$id];
        $sql = "SELECT image FROM $plural WHERE id = ?";
        ...
        $sql = "DELETE FROM $plural WHERE id = ?";
    ...
    elseif(isset($_POST['deleteSelected']))
    {
        ...
        $sql2 = "SELECT image FROM $plural WHERE id IN ($placeholders)";
        ...
        $sql = "DELETE FROM $plural WHERE id IN ($placeholders)";
        $result = $obj->executeSQL($sql, $paramList);
        $affectedRows = $result['affectedRows'];
